...
def main():
    ...
    x_val_norm_set = x_norm_set[seed_1[math.floor(0.8 * len(x_norm_set)): len(x_norm_set)]]
    y_val_norm_set = y_norm_set[seed_1[math.floor(0.8 * len(x_norm_set)): len(x_norm_set)]]
    batch_size = 512
    ...
    model = modeling_new.Head_Motion_Predict().cuda()
    loss = nn.L1Loss().cuda()
    #loss1 = nn.L1Loss()
